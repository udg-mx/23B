const catalog = {
    ...
    init: function() {
        ...
        // event listener eliminar todos los productos
        document.getElementById("removeProductsBtn").addEventListener("click", () => {
    ...
    remove: function(productId) {
        ...
        const rowToDelete = document.querySelector(`#productTable tbody tr[data-product-id="${productId}"]`);

        // si la fila existe, la elimina
        if (rowToDelete)
        {
            rowToDelete.remove();
        }
        ...
        {
            console.error(`No row found with product ID: ${productId}`);
        }

        // si ya no hay productos, renderiza la tabla para mostrar el mensaje de "no hay productos"
        if (this.products.length === 0) this.render();
    ...
    render: function() {
        ...
        if (this.products.length === 0)
        {
            ...
            this.products.forEach(product => {
                ...
                const cellClass = 'px-6 py-4 whitespace-no-wrap font-medium text-gray-500 text-sm leading-5';

                // crea una fila y le agrega clases de tailwind
                const row = document.createElement("tr");
                row.className = 'hover:bg-gray-50 transition duration-150 even:bg-gray-50 odd:bg-gray-100';
                ...
                const appendToRow = (data, orientation) =>
                {
                    ...
                    cell.className = cellClass + (orientation === 'end' ? ' text-end' : '');
                ...
                const deleteBtnIcon = document.createElement('i'); // crea elemento i
                deleteBtnIcon.className = 'fas fa-trash-alt text-red-500 hover:text-red-700'; // agrega clases al icono
                deleteBtn.appendChild(deleteBtnIcon); // agrega el icono al botón
                deleteBtn.className = 'p-2 rounded hover:bg-red-100'; // agrega clases al botón
                deleteBtn.dataset.productId = product.id; // agrega el id del producto al botón

                // crea celda para el botón
                const cell = document.createElement("td"); // crea elemento td
                cell.className = cellClass + ' text-center'; // agrega clases a la celda
                cell.appendChild(deleteBtn); // agrega el botón a la celda
            ...
            // agrega event listener al botón de eliminar siempre que tenga data-product-id
            document.querySelectorAll('#productTable tbody tr td button[data-product-id]').forEach(button => {
                button.addEventListener('click', () => {

                    if (confirm(`¿Estás seguro de eliminar este producto (${button.dataset.productId})?`))
                    {
