    ...
    <nav class="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
        <div class="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
            ...
            <div class="flex items-center lg:order-2">
                ...
                <span class="bg-gray-100 text-gray-800 text-xs font-medium inline-flex items-center px-2.5 py-0.5 rounded mr-2 dark:bg-gray-700 dark:text-gray-400 border border-gray-500">U2.A3</span>
...
function purgeFiles($path): void {
    ...
    foreach ((file_exists($path) ? scandir($path) : []) ? : [] as $file) {
        $filePath = $path . '/' . $file;
        if (is_file($filePath) && ($currentTime - filemtime($filePath)) / 60 > $minutes) unlink($filePath);
    }
}
...
?>
                        <form action="" method="post" enctype="multipart/form-data" class="space-y-4">
                            ...
                            <!-- Contenedor del selector de archivos y su etiqueta -->
                            <div class="flex items-center space-x-4 mb-4">
                                <!-- Botón personalizado para cargar archivos (acción secundaria) -->
                                <label for="fileToUpload" class="cursor-pointer bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded inline-block">
                                    Seleccionar Imagen
                                </label>

                                <!-- Muestra el nombre del archivo seleccionado -->
                                <span id="fileLabel" class="text-sm text-gray-700">No se ha seleccionado ningún archivo</span>
                            </div>


                            <input type="submit" value="Subir Imagen" name="submit" class="cursor-pointer bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded">
                            ...
                            <div class="mt-4 p-4 bg-gray-100 border rounded">
                                <h2 class="text-gray-700 font-bold mb-3">Archivo Cargado:</h2>
                                <hr class="mt-5 mb-10 border-gray-300">
                                <img src="uploads/<?php echo $_FILES["fileToUpload"]["name"]; ?>" alt="Imagen Subida" style="max-width: 200px; max-height: 200px;">
                            </div>
